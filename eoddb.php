...
if ($db) {
        ...
        if ($answer) {
            ...
            if ($completepercentage_from_db + $completepercentage[$i] <= 100) {
                ...
                if (mysqli_query($db, $update)) {
                    ...
                    echo json_encode(['code' =>404, 'status' => 'error', 'message' => 'Database Insertion failure']);
                    break;
                }
            } else {
                $success = 1;
    ...
    if ($success === 1) {
        echo json_encode(['code' =>200, 'status' => 'success', 'message' => 'You have successfully submitted your end of the day report', 'error_msg' => $error_msg]);
    }
} else {
    echo json_encode(['code' =>404, 'status' => 'error', 'message' => 'Database connection failure']);
}
?>